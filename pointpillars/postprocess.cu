...
PostprocessCuda::PostprocessCuda(const int num_threads, const float float_min, const float float_max,
    const int num_class,const int num_per_cls,
...
void PostprocessCuda::DoPostprocessCuda(
    ...
    GPU_CHECK(cudaMemcpy(host_box, box_preds, num_output_box_feature_ * num_per_cls_ * sizeof(float), cudaMemcpyDeviceToHost));
    GPU_CHECK(cudaMemcpy(host_score, box_scores, num_class_ * num_per_cls_ * sizeof(float), cudaMemcpyDeviceToHost));
    ...
    for (int class_idx = 0; class_idx < num_class_; ++ class_idx) {  // hardcode for class_map as {0, 12 , 34 , 5 ,67 ,89}
        ...
        for (size_t idx = 0 ; idx < num_per_cls_ ; idx++)
        {
            ...
            if (score > score_threshold_ && host_filtered_count[class_idx] < nms_pre_maxsize_)  // filter out boxes which threshold less than score_threshold
            {
        ...
        for (int ith_box = 0 ; ith_box  < host_filtered_count[class_idx] ; ++ith_box) 
        {
            ...
            host_sorted_filtered_box[ith_box * 7 + 0] = host_filtered_box[ith_box * 7 + 0];
            host_sorted_filtered_box[ith_box * 7 + 1] = host_filtered_box[ith_box * 7 + 1];
            host_sorted_filtered_box[ith_box * 7 + 2] = host_filtered_box[ith_box * 7 + 2];
            host_sorted_filtered_box[ith_box * 7 + 3] = host_filtered_box[ith_box * 7 + 3];
            host_sorted_filtered_box[ith_box * 7 + 4] = host_filtered_box[ith_box * 7 + 4];
            host_sorted_filtered_box[ith_box * 7 + 5] = host_filtered_box[ith_box * 7 + 5];
            host_sorted_filtered_box[ith_box * 7 + 6] = host_filtered_box[ith_box * 7 + 6];
        }
